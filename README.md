...
Once the runtime starts, you can access the project at [http://localhost:8080/demojk](http://localhost:8080/demojk).
...
```
./mvnw clean package
docker build -t demojk:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 demojk:v1
```
